...
SCHOOLAR_FOLDER_PATH: str = os.path.join("data", "process", "schoolar")

ECONOMIC_2021_FOLDER_PATH: str = os.path.join("data", "process", "economic", "2021")
ECONOMIC_2022_FOLDER_PATH: str = os.path.join("data", "process", "economic", "2022")
ECONOMIC_2023_FOLDER_PATH: str = os.path.join("data", "process", "economic", "2023")

OUTPUT_FOLDER_PATH: str = os.path.join("data", "process", "complete")
...
def create_complete_files(
        ...
        schoolar_folder_path: str = SCHOOLAR_FOLDER_PATH,
    ...
    for state in BRAZIL_STATES:
        ...
        schoolar_data: dict = read_records(
            folder_path=schoolar_folder_path,
            state_abbr=state_abbr,
        )

        print("0" * 50)
        print(len(economic_2021_data))
        print(len(economic_2022_data))
        print(len(economic_2023_data))
        print("=" * 50)
        print(len(schoolar_data))
        print("0" * 50)

        
        complete_data: dict = {}
        city_codes: list[str] = list(economic_2021_data.keys())
        for city_code in city_codes:
            economic_data: dict = {
                "2021": economic_2021_data[city_code],
                "2022": economic_2022_data[city_code],
                "2023": economic_2023_data[city_code],
            }
            schoolar_records: list = []
            for record in schoolar_data:
                if record["id_municipio"] == city_code:
                    schoolar_records.append(
                        record
                    )
            
            complete_data[city_code] = {
                "region_code": economic_2021_data[city_code]["region_code"],
                "region_name": economic_2021_data[city_code]["region_name"],
                "state_code": economic_2021_data[city_code]["state_code"],
                "state_abbr": economic_2021_data[city_code]["state_abbr"],
                "state_name": economic_2021_data[city_code]["state_name"],
                "city_code": economic_2021_data[city_code]["city_code"],
                "city_name": economic_2021_data[city_code]["city_name"],
                "schoolar": schoolar_records,
...
create_complete_files(
    ...
    schoolar_folder_path=SCHOOLAR_FOLDER_PATH,
